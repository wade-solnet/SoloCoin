...
import "./SoloCToken.sol";

/**
   @title Market Validation Mechanism (MVM)
 */
contract SoloCMarketValidationMechanism is Ownable {
  using SafeMath for uint256;

  // The SoloC token contract
  SoloCToken public soloCToken;

  // The address of the foundation wallet. It can claim part of the eth funds
  // following an exponential curve until the end of the MVM SoloCetime (24 or 48
  ...
  // The price at which the MVM buys tokens at the beginning of its SoloCetime
  ...
  /**
     ...
     @param SoloCAddr the SoloC token address
    ...
    */
  function SoloCMarketValidationMechanism(
    address SoloCAddr, uint256 _startTimestamp, uint256 _secondsPerPeriod,
    uint8 _totalPeriods, address _foundationAddr
  ) {
    require(SoloCAddr != address(0));
    require(_startTimestamp > block.timestamp);
    require(_secondsPerPeriod > 0);
    require(_totalPeriods == 24 || _totalPeriods == 48);
    require(_foundationAddr != address(0));

    soloCToken = SoloCToken(SoloCAddr);
  ...
  function fund() public payable onlyOwner {
    ...
    originalTotalSupply = soloCToken.totalSupply();
  ...
  /**
     @dev Change the SoloCToken address
    */
  function changeToken(address newToken) public onlyOwner {
    soloCToken = SoloCToken(newToken);
  }
  ...
  /**
     ...
     very low amounts ending with higher chunks at the end of the MVM SoloCetime
    */
  function calculateDistributionPeriods() public {
    ...
    // is used when MVM SoloCetime is 24 months, deltas48 when it's 48 months.
  ...
  /**
     ...
     @return the current buy price (in eth/SoloC, multiplied by PRICE_FACTOR)
    */
  ...
  function getMaxClaimableWeiAmount() public view returns (uint256) {
    if (isFinished()) {
      ...
      uint256 currentCirculation = soloCToken.totalSupply();
      uint256 accumulatedDistributionPercentage = getAccumulatedDistributionPercentage();
  ...
  function sendTokens(uint256 tokens) public whenNotPaused {
    ...
    soloCToken.transferFrom(msg.sender, address(this), tokens);
    soloCToken.burn(tokens);
  ...
  /**
     @dev Returns whether the MVM end-of-SoloCe has been reached. When that
     happens no more tokens can be sent to the MVM and the foundation can claim
     100% of the remaining balance in the MVM

     @return true if the MVM end-of-SoloCe has been reached
    */
  ...
  /**
     ...
     claimed from the MVM while paused. MVM total SoloCetime is extended by the
