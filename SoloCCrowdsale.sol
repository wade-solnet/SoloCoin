...
import "./SoloCMarketValidationMechanism.sol";



/**
   @title Crowdsale for the SoloC Token Generation Event

   Implementation of the SoloC Token Generation Event (TGE) Crowdsale: A 2 week
 ...
 */
contract SoloCCrowdsale is Ownable, Pausable {
  ...
  // Quantity of SoloC that is received in exchage of 1 Ether during the first
  // week of the 2 weeks TGE
  uint256 public rate1;

  // Quantity of SoloC that is received in exchage of 1 Ether during the second
  // week of the 2 weeks TGE
  ...
  // Amount of SoloC minted and transferred during the TGE
  ...
  SoloCMarketValidationMechanism public MVM;
  ...
  **/
  ...
  /**
     @dev Returns the current SoloC per Eth rate during the TGE

     @return the current SoloC per Eth rate or 0 when not in TGE
   */
  ...
  /**
     ...
     SoloC using the current rate obtained by calling `getRate()`.

     @param beneficiary Address to which SoloC should be sent
   */
  ...
  /**
     ...
     @param beneficiary Address to which SoloC will be sent
     @param weiSent Amount of wei contributed
     @param rate SoloC per ether rate at the moment of the contribution
   */
  ...
  function forwardFunds(bool deployMVM) internal {
    ...
    if ((weiRaised <= foundationBalanceCapWei) || !deployMVM) {
      ...
      MVM = new SoloCMarketValidationMechanism(
  ...
  /**
     ...
     12.8% of tokens in a 4y (1y cSoloCf) vesting schedule.
